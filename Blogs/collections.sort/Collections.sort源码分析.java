...
binarySort要做的就是把后续的元素依次插入到属于他们的位置，基点就是已经排好序的子数组（如果没有的子数组就是首元素），把当前操作的元素称为pivot
，通过二分查找，找到自己应该插入的位置（达到的状态是left==right），找到位置后，就要为pivot的插入腾出空间，所以需要元素的移动，代码中如果移动
少于两个元素就直接操作，否则调用System.arraycopy()，最后插入我们的pivot到正确的位置。
...
得到 minRun 之后，取 minRun 和 nRemaining 的最小值作为这次要排序的序列，初始的有序数组和前面情况（1）的获取方式一样，然后做一次二分插入排序，
现在有序序列的长度是force，这一部分排好序之后，把本次run的起始位置和长度存入一个stack中（两个数组），后续就是根据这些区间完成排序的。
每次push之后就是要进行合并检查，也就是说相邻的区间能合并的就合并，具体的：
...
首先把 run length更新到 ruLen[i] 中，删掉 i+1 的run信息；接下来定位区间2的最小元素在有序区间1的插入位置，更新区间1的 run base 和 run length,
称更新后的为区间1'; 然后查找区间1'的最大元素在区间2的正确定位；此时此刻这个数组已经得到了有效的划分，如下图，只需要合并[base1,len1]和[base2,len2]就可以了，
其他段已经在正确位置。
...
为了性能，在len1<=len2的时候使用mergeLo，len1>=len2的时候使用mergeHi，通过前面的定位，到这里的时候，有a[base1]>a[base2],a[base1+len1]<a[base2+len2]，
合并后，最终完成了排序。
